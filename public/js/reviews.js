...
async function loadReviews() {
  ...
  try {
    ...
    if (!reviews.length) {
      list.innerHTML = `<div class="reviews-empty"><p>No reviews yet — be the first to leave one below!</p></div>`;
      return;
    }
    const avg = (reviews.reduce((s, r) => s + r.rating, 0) / reviews.length).toFixed(1);
    list.innerHTML = `
      <div class="reviews-page-summary">
        <span class="reviews-stars-big" style="color:#00b67a">${'★'.repeat(Math.round(avg))}${'☆'.repeat(5 - Math.round(avg))}</span>
    ...
    list.innerHTML = `<div class="empty-state"><p>Failed to load reviews. Please try refreshing.</p></div>`;
  }
}
...
async function submitReview(e) {
  ...
  try {
    const res = await fetch('/api/reviews/submit', {
      ...
      body: JSON.stringify({ author_name: name, class_attended: attended, body })
    });
    if (!res.ok) throw new Error();
    document.getElementById('review-submit-form').reset();
    showMsg('Thank you! Your review has been submitted and will appear once approved.', 'success');
  } catch {
