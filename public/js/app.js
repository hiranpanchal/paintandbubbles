...
function getFontStack(name) {
  ...
  const script = ['Dancing Script','Pacifico','Caveat','Satisfy','Great Vibes','Lobster'];
  if (script.includes(name))  return `'${name}', cursive`;
...
function renderEventCard(event) {
  ...
  // Listing cards don't surface a live spot count — only flag the sold-out case
  // so customers don't click in to find no availability. Capacity detail lives
  ...
  document.getElementById('event-modal-body').innerHTML = `
    ...
    <div class="event-detail-body">
      ...
      <div class="event-detail-footer">
        ...
        ${isSoldOut
          ? `<button class="btn btn-outline" onclick="openWaitlist(${event.id}, '${event.title.replace(/'/g,"\\'")}')">Join Waitlist</button>`
          : `<button class="btn btn-primary" onclick="openBooking(${event.id})">Book Now →</button>`
        }
      </div>
    </div>`;
...
async function submitWaitlist() {
  ...
  try {
    ...
    modal.querySelector('.waitlist-form').innerHTML = `
      <div style="text-align:center;padding:24px 0">
        <div style="font-size:48px;margin-bottom:12px;">🎉</div>
        <p style="font-size:17px;font-weight:800;color:var(--text-dark);margin-bottom:8px;">You're on the waitlist!</p>
        <p style="font-size:14px;color:var(--text-light);line-height:1.6;">We'll email you straight away if a spot opens up. We've also sent you a confirmation email.</p>
        <button class="btn btn-primary" style="margin-top:20px;padding:12px 28px;font-size:14px;" onclick="closeModal('waitlist-modal')">Close</button>
      </div>`;
  } catch (err) {
