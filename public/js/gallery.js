...
// ---- GALLERY PAGE ----
function renderGalleryPage(s) {
  // Hero
  const heroTitle = document.getElementById('gallery-hero-title');
  if (heroTitle) heroTitle.textContent = s.gallery_hero_title || 'Our Gallery';

  const heroSub = document.getElementById('gallery-hero-sub');
  if (heroSub) heroSub.textContent = s.gallery_hero_sub || 'A glimpse of the creativity from our sessions';
  ...
  let images = [];
  try { images = JSON.parse(s.gallery_images || '[]'); } catch {}

  if (!images.length) {
    grid.innerHTML = '<div class="gallery-empty">No images yet — check back soon!</div>';
    return;
  }
