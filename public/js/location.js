/* =============================================
   PAINT & BUBBLES — LOCATION PAGES
   Handles /coventry, /leamington-spa, /solihull
   ============================================= */

const CITY_MAP = {
  'coventry':       { name: 'Coventry',       search: 'Coventry',    county: 'West Midlands' },
  'leamington-spa': { name: 'Leamington Spa', search: 'Leamington',  county: 'Warwickshire'  },
  'solihull':       { name: 'Solihull',        search: 'Solihull',    county: 'West Midlands' },
};
...
document.addEventListener('DOMContentLoaded', async () => {
  if (!city) return; // shouldn't happen but guard anyway

  // Populate all city name placeholders
  document.querySelectorAll('#loc-city-name, .loc-city-inline').forEach(el => {
    el.textContent = city.name;
  });
  const eyebrow = document.getElementById('loc-eyebrow');
  if (eyebrow) eyebrow.textContent = city.name + ' · Paint & Bubbles';

  await Promise.all([loadEvents(), applyDesignSettings()]);
});

// ─── Events ──────────────────────────────────────────────────────────────────

async function loadEvents() {
  const container = document.getElementById('loc-events-container');
  try {
    const events = await fetch('/api/events').then(r => r.json());
    const now    = new Date();

    // Filter to this city and future events only
    const cityEvents = events.filter(e => {
      if (!e.is_active) return false;
      if (e.date && new Date(e.date) < now) return false;
      const loc = (e.location || '').toLowerCase();
      return loc.includes(city.search.toLowerCase());
    });

    if (!cityEvents.length) {
      container.innerHTML = `
        <div class="loc-no-events">
          <div class="loc-no-events-icon">🎨</div>
          <h3>No upcoming events in ${city.name} right now</h3>
          <p>We're adding new dates all the time — check back soon, or browse all our events across every location.</p>
          <a href="/events" class="btn btn-primary">Browse All Events</a>
        </div>`;
      return;
    }

    container.innerHTML = `<div class="loc-events-grid">${cityEvents.map(renderEventCard).join('')}</div>`;
  } catch {
    container.innerHTML = `
      <div class="loc-no-events">
        <div class="loc-no-events-icon">⚠️</div>
        <h3>Couldn't load events</h3>
        <p>Please try refreshing, or <a href="/events" style="color:var(--rose)">view all events</a> instead.</p>
      </div>`;
  }
}

function renderEventCard(e) {
  const price    = e.price_pence ? `£${(e.price_pence / 100).toFixed(0)}` : 'Free';
    ...
    <a href="/events/${e.slug || e.id}" class="loc-event-card">
      ...
      <div class="loc-event-body">
        <div class="loc-event-badges">
          ${e.category ? `<span class="loc-event-badge">${escHtml(e.category)}</span>` : ''}
          ${badge}
        </div>
        <div class="loc-event-title">${escHtml(e.title)}</div>
        <div class="loc-event-meta">
          ${dateStr ? `📅 ${dateStr}` : ''}${timeStr ? ` · ${timeStr}` : ''}<br>
          📍 ${escHtml(e.location || '')}
        </div>
        <div class="loc-event-footer">
          <div class="loc-event-price">${price} <span style="font-size:13px;font-weight:600;color:var(--text-light)">pp</span></div>
          ${soldOut
            ? '<span style="font-size:13px;font-weight:700;color:#9E8E96">Sold Out</span>'
            : '<span class="loc-event-book">Book Now</span>'}
        </div>
      </div>
    </a>`;
}
...
async function applyDesignSettings() {
  try {
    const s = await fetch('/api/design/settings').then(r => r.json());
    if (!s) return;
