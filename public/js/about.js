...
function renderAboutPage(s) {
  // Hero
  const heroTitle = document.getElementById('aboutpage-hero-title');
  if (heroTitle) heroTitle.textContent = s.aboutpage_hero_title || 'About Us';

  const heroSub = document.getElementById('aboutpage-hero-sub');
  if (heroSub) heroSub.textContent = s.aboutpage_hero_sub || 'The story behind Paint & Bubbles';

  // Label and title
  const label = document.getElementById('aboutpage-label');
  if (label) label.textContent = s.aboutpage_label || 'Our Story';

  const title = document.getElementById('aboutpage-title');
  if (title) title.textContent = s.aboutpage_title || 'Where creativity meets good company';

  // Body paragraphs
  const bodyEl = document.getElementById('aboutpage-body');
  if (bodyEl) {
    const paras = [s.aboutpage_body_1, s.aboutpage_body_2, s.aboutpage_body_3]
      .filter(p => p && p.trim())
      .map(p => `<p class="about-body">${escHtml(p)}</p>`)
      .join('');
    bodyEl.innerHTML = paras;
  }

  // Pillars
  const pillarsEl = document.getElementById('aboutpage-pillars');
  if (pillarsEl) {
    const pillarItems = [1, 2, 3].map(n => ({
      title: s[`aboutpage_pillar_${n}_title`] || '',
      text:  s[`aboutpage_pillar_${n}_text`]  || '',
    }));
    const hasAny = pillarItems.some(p => p.title);
    if (hasAny) {
      pillarsEl.innerHTML = pillarItems
        .filter(p => p.title)
        .map(p => `
          <div class="about-pillar">
            <div class="about-pillar-title">${escHtml(p.title)}</div>
            <div class="about-pillar-text">${escHtml(p.text)}</div>
          </div>`)
        .join('');
    }
  }
